...
public class Chat2Controller {
    ...
    @Resource
    private Chat2Service chatService;
    ...
    public String chatView(Model model,
        ...
        List<DialogDTO> dialogs = chatService.listDialog(sessionId).getData();
    ...
    protected void saveData(HttpServletRequest request, HttpServletResponse response)
        ...
        DialogDTO dialogResult = chatService.chat(dialog).getData();
    }

    @PostMapping("/deleteData2")
    protected void deleteData(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String id = request.getParameter("id");
        // 在这里处理请求，将 data 存入后台数据列表中
        String sessionId = packSessionId(request, response);
        boolean del = chatService.deleteDialog(sessionId, Integer.valueOf(id)).getData();
        if (!del) {
    ...
    public String monitor(Model model,
                          HttpServletRequest request) {
        List<DialogDTO> dialogs = chatService.listDialog(ChatmeContants.ALL_SESSION_ID).getData();
        model.addAttribute("dialogs", dialogs);
        return "monitor";
    }
